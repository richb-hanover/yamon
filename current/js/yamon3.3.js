...
var g_nobwCap
var g_Settings={}, g_IPs={}, g_Restrictions={}
...
$(document).ready(function (){
  ...
  $('._updatefreq').parents('p').append("<br/>Last update:<a id='last_update' href='"+_liveFileName+"' target='_blank'></a>")
  ...
  .success(function(data){
    ...
    $('#SummaryUsageTable th:first').attr('colspan',1)
    $('#SummaryUsageTable tr:first').append('<th class="br bt">Router: '+router+'<br/>Firmware: '+firmware+'</th>').append('<th class="br bt">Installed: '+installed+'<br/>Updated: '+updated+'</th>')
    if (!g_Settings['router']){
      var gm = getMessage('save router')
      gm.done(function () {
        $('.dismiss').hide()
        $('[name="shri"]').change(function(){ 
          $('.dismiss').fadeIn('slow').click(function(){
            if($('#sh-y').is(':checked')){
              $.ajax({
        ...
        var gm = getMessage('updated router')
        gm.done(function () {
        $('.dismiss').hide()
        $('[name="shri"]').change(function(){ 
          $('.dismiss').fadeIn('slow').click(function(){
...
function loadMonthly(){
  var failcount=0
  function getMonthlyData(fn){
    var md = $.Deferred()
    $.getScript(fn)
    .done(function(dlist,textStatus){
      if(dlist==''){
        var olddatafile=dp+yr+'-'+rm+'-'+rd+'-'+_usageFileName
        failcount++
        if (failcount>2){
          console.log('failed - loadMonthly',jqxhr,settings,exception)
          md.fail()
        }
        else{
          var gmd2 = getMonthlyData(olddatafile)
          gmd2.done(function(){
            md.resolve()
          }).fail(function(){
            md.fail()
      ...
      flushChanges()
      $('#MonthlyHeader .icon').attr('title', 'View the contents of the monthly usage data file').data('link', fn)
      var cdown=0,cup=0
    ...
    .fail(function(jqxhr,settings,exception){
      ...
      if (failcount>2){
        console.log('failed - loadMonthly',jqxhr,settings,exception)
        clearLoading(1)
        getMessage('monthly not found', olddatafile)
        md.fail()
      }
      else{
        var gmd2 = getMonthlyData(olddatafile)
  ...
  var dp=_wwwData+(_organizeData==0?'':(_organizeData==1?yr+'/':yr+'/'+rm+'/'))
  monthly=[]
  zeroDevicesTotal()
  zeroMonthlyTotals()
  if(cleardata)$('#MonthlyBreakdown,#breakdownFooter').html('')
    
  var datafile=dp+yr+'-'+rm+'-'+_usageFileName
  var gmd = getMonthlyData(datafile)
  gmd.done(function(){
    deferred.resolve()
  }).fail(function(){
...
function loadHourly(cleardata){
  ...
  var logfile='logs/monitor-'+yr+'-'+mo+'-'+da+'.log'
  ...
  .done(function(dlist,textStatus){
    ...
    $('#DailyUsageHeader .log').attr('title', 'View the contents of the hourly usage data file').data('link', logfile)
    ...
    updateDashboard()
    var ry=_rs_Date.getFullYear(),rm=twod(_rs_Date.getMonth()-(-1)),rd=twod(_ispBillingDay),bill=ry+'-'+rm+'-'+rd,mts=monthly_totals.up+';'+monthly_totals.down
    if(_unlimited_usage=='1'){
      mts+=';'+monthly_totals.ul_up+';'+monthly_totals.ul_down
    }
    //if($('#bandwidth-l').is(':checked')) mts+=';'+$('#bw_cap').val()
    ...
    Object.keys(devices).sort(byName).forEach(function(d){
      if(!monthly[d]) return
      var id=devices[d].id,u_n=devices[d].cg,ud=devices[d].name,u_d=devices[d].cn
      $("#mb-filter").append($("<option/>").attr('id','mbd-'+d).attr('title',d+' | '+devices[d].ip).data('gp',devices[d].group).attr('value','dd-'+d).text(ud).addClass('ddl-d du-'+u_n))
    })
...
function setReportDates(ri){
  ...
  if(!ri){
    ...
    $('#p-timer').fadeIn('slow')
    _rs_Date=new Date(yr,mo-(da>=_ispBillingDay?1:2),_ispBillingDay)
    _re_Date=new Date(yr,mo-(da>=_ispBillingDay?0:1),_ispBillingDay-1,23,59,59)
  }
  else{
    var de=ri.split('-')
    da=twod(de[2])
...
function serverloads(minL,minTS,maxL,maxTS){
  minL*=1
  maxL*=1
  var gw=$('.gradient').css('width').replace('px','')*1 ,wm=gw/Math.max(_processors*2,$('#sp_maxSL').text()*1)
...
function pnd(arr){
  ...
  var svd=!sut?0:arr.down-p_pnd_d,svu=!sut?0:arr.up-p_pnd_u,sdr=!sut?0:arr.iptables.DROP-p_dropped, wl=!arr.lost?0:arr.lost, loc=!sut?0:arr.iptables.local-p_local
  p_dropped=(!arr.iptables)||(!arr.iptables.DROP)?0:arr.iptables.DROP
  p_local=(!arr.iptables)||(!arr.iptables.p_local)?0:arr.iptables.local
  if(sut>o_sut||!o_sut){
    ...
    svd=arr.down
    svu=arr.up
    sdr=arr.iptables.DROP
    pnd_data.usage[arr.hour*1]={down:svd,up:svu,restarted:true,lost:wl,dropped:sdr,local:loc}
  }
  if(!arr['hr-loads']){}
  else{
    var hl=[arr.hour]
  ...
  pnd_data.total.dropped+=sdr
  pnd_data.total.lost+=wl
  pnd_data.total.local+=loc
  p_pnd_d=arr.down
  p_pnd_u=arr.up
  o_sut=arr.uptime
}
...
function setDailyTab(){
  ...
  $('.cf-desc').text(cddesc)
  $('#cf-desc').val(cddesc)
  $('.cf-u').data('value',cdu*g_toMB)
  $('.cf-d').data('value',cdd*g_toMB)
  $('#cf-u').val(cdu)
  $('#cf-d').val(cdd)
  ...
  Object.keys(hourly).sort(byGN).forEach(function(k){
    ...
    ul_dtot+=ul_down
    if($("#"+gid).length==0){
      var nr=$('#blank-group-row').clone(true,true).detach().attr('id',gid).data('g-n',gn).addClass('is_u').removeClass('hidden')
    ...
    updateRow(gid,arr)
    if($("#"+did).length==0){
      var nr=$('#blank-device-row').clone(true,true).detach().attr('id',did).attr('data-mac',k).data('g-n',gn+'-'+ddn).addClass('is_d '+gid).data('group',devices[k].group).removeClass('hidden')
      ...
      nr.find('.deviceName').attr('title',k+' | '+devices[k].ip)
  ...
  var arr=[[' .downloads',pnd_data.total.local],[' .uploads',0],[' .tByts',pnd_data.total.local]]
...
function setMonthlyTab(){
  ...
  Object.keys(monthly).sort(byGN).forEach(function(k){
    ...
    ul_dtot+=ul_down
    if($("#"+gid).length==0){
      var nr=$('#blank-group-row').clone(true,true).off('click').off('dblclick').detach().attr('id',gid).data('g-n',gn).addClass('is_u').removeClass('hidden')
      nr.find('.userName').text(devices[k].group)
    ...
    if($("#"+did).length==0){
      ...
      nr.find('.deviceName').attr('title',k+' | '+devices[k].ip).off('click').off('dblclick')
      nr.find('.thedevice').text(devices[k].name)
      nr.insertAfter('#'+gid)
    }
    arr=[[' .downloads',down],[' .uploads',up],[' .ul-down',ul_down],[' .ul-up',ul_up],[' .tByts',ut]]
  ...
  if(inc_isp){
    ...
    if(!g_Settings['isp'][mo+'-'+yr]){
      ...
      var isp_totals=JSON.parse(g_Settings['isp'][mo+'-'+yr]),isp_d=0,isp_u=0,mt=$('#MonthlyFooter .tByts').data('value')
...
function activeConnections(){
  ...
  var slic=$('#show-local').is(':checked')
  $(curr_connections).sort(byIP).each(function (a, b) {
    if (!b[1] || b[0]=='unknown') return
    ...
    if (filterIP == '' || filterIP == sip || filterIP == dip) {
      ...
      cs = '',
      cn = '',
      ipfnd = g_IPs[dip] ? ' ipfnd' : ''
      if (!g_IPs[dip]) {
        iptxt = dip
      }
      else {
        var ipv = g_IPs[dip].split('|')
        iptxt = ipv[0] + (ipv[2] == '' ? '' : '<br/>' + ipv[2])
        cs = ipv[1].replace(/ /g, '-') || 'f-unk'
        cn = ipv[1]
      }
      var is_internal=(dip.indexOf($('#acc-filter-ip').val())>=0||dip.indexOf('255.255.255')>=0),slon=''
      if(is_internal){
        if(slic)
          cs='home'
        else
      ...
      var nr = $('#blank-acon-row').clone(true,true).removeAttr('id').removeClass('hidden').addClass(slon)
      nr.find('.conn').text(b[0])
      nr.find('.src-ip').text(ip2device[sip] || 'unknown: ' + sip)
      nr.find('.sprt').text(b[2])
      nr.find('.dest-ip').attr('title', dip).html(iptxt).addClass(ipfnd)
      nr.find('.flag').addClass(cs).attr('title', cs)
...
function curr_users(arr){
  ...
  var mac=arr.mac.toUpperCase(), dg=!devices[mac]?'Unknown group':devices[mac].group, dn=!devices[mac]?'Unknown device':devices[mac].name
...
function curr_users_totals(tt){
  ...
  nr.find('.td-time').text(numLU + '. Totals').attr('title', last_update + ' / ' + dt )
...
function setDevices(){
  ...
  Object.keys(devices).sort(byName).forEach(function(d){
    ...
    if($('#dd-'+id).length==0){
      ...
      var nr=$('#blank-devices-row').clone(true,true).detach().attr('id','dd-'+id).attr('data-mac',d).addClass('is_dd c-p'+is_z+cs_edit + dih).removeClass('hidden')
